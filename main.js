...
// ==============================================
// 3. SCANNER QR CODE (VERSIONE OTTIMIZZATA CON TIMEOUT)
// ==============================================

// Costanti per la configurazione
const QR_SCAN_TIMEOUT = 30000; // 30 secondi
const SCAN_INTERVAL = 200; // 200ms tra una scansione e l'altra

// Variabile globale per tenere traccia dello stato del preload
let qrScannerReady = false;

// Funzione per il preload opzionale (da chiamare in anticipo se desiderato)
...
// Funzione per il caricamento della libreria
...
async function scanQRCode() {
  ...
  try {
    // Carica la libreria jsQR solo se non è già presente
    if (!window.jsQR) {
      try {
        await loadJSQRLibrary();
      } catch (error) {
        loader(); // Chiude il loader
    ...
    // Crea l'interfaccia scanner con timer
    ...
    try {
      ...
      return new Promise((resolve) => {
        ...
        const timerInterval = setInterval(() => {
          ...
          const remaining = Math.ceil((timeoutId._idleStart + QR_SCAN_TIMEOUT - Date.now()) / 1000);
        ...
        // Avvia la scansione quando il video è pronto
