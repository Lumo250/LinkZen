...
async function loadCategories() {
  try {
    ...
    // Se il file non esiste, carica da GitHub e salva localmente
    const defaultResponse = await fetch('https://raw.githubusercontent.com/yourrepo/LinkZen/main/default-categories.json');
    ...
    return {
      defaultCategories: [...], // le attuali categorie hardcoded
      userCategories: []
    };
  } catch (error) {
...
document.getElementById("export-basic").addEventListener("click", async () => {
  const { visitedUrls = [] } = await storage.get({ visitedUrls: [] });
  const blob = new Blob([JSON.stringify({ visitedUrls }, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
...
document.getElementById("export-full").addEventListener("click", async () => {
  const { visitedUrls = [], keywordToCategory = {} } = await storage.get({ 
    visitedUrls: [], 
    keywordToCategory: {} 
  });
  const blob = new Blob([JSON.stringify({ visitedUrls, keywordToCategory }, null, 2)], { 
...
document.getElementById("import-default-btn").addEventListener("click", async () => {
  ...
  try {
    const response = await fetch('https://lumo250.github.io/LinkZen/default-categories.json');
    if (response.ok) {
    ...
    showAlert("Error", "Failed to import default categories");
  }
...
async function updateDefaultCategories(updatedCategories) {
  const categoriesData = await loadCategories();
  categoriesData.defaultCategories = updatedCategories;
...
async function addUserCategory(newCategory) {
  const categoriesData = await loadCategories();
  
  if (!categoriesData.userCategories.includes(newCategory)) {
    categoriesData.userCategories.push(newCategory);
    await saveCategories(categoriesData);
...
async function loadUrls() {
  ...
  const { defaultCategories, userCategories } = await loadCategories();
