...
// Preserva lo stato della finestra categorie se aperta
  const categoriesPopup = document.getElementById('categories-popup');
  if (!categoriesPopup.classList.contains('hidden')) {
    const searchTerm = document.getElementById('new-category-input').value;
    setTimeout(() => {
      renderCategoriesList(searchTerm);
      // Riposiziona il popup
      const input = document.getElementById('new-category-input');
      const rect = input.getBoundingClientRect();
      categoriesPopup.style.top = `${rect.bottom + window.scrollY + 5}px`;
      categoriesPopup.style.left = `${rect.left + window.scrollX}px`;
    }, 300); // Aspetta che la transizione del tema sia completata
  }
  });
...
document.getElementById("categories-btn").addEventListener("click", async () => {
  ...
  // Nascondi la box IA se visibile
  ...
  // Aggiungi titolo per le categorie predefinite
  ...
  defaultCategories.forEach(category => {
    ...
    chip.style.padding = "4px 8px";
    chip.style.backgroundColor = document.body.classList.contains("dark") ? "#333" : "#eee";
    chip.style.borderRadius = "4px";
  ...
  if (userCategories.length === 0) {
    ...
    userCategories.forEach(category => {
      ...
      chip.textContent = category;
      chip.style.padding = "4px 8px";
      chip.style.backgroundColor = document.body.classList.contains("dark") ? "#2c3e50" : "#2196F3";
