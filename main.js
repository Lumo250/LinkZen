...
document.getElementById("import-custom").addEventListener("click", () => {
  ...
  resetFileInput(); // Aggiungi questa linea
  ...
  reader.onload = async (e) => {
    try {
      ...
      if (data.visitedUrls && Array.isArray(data.visitedUrls)) {
        // Chiudi tutte le finestre prima dell'importazione
        ...
        // Forza il refresh completo dell'interfaccia
        await Promise.all([
          showCategories(),
          renderIAKeywords(),
          loadUrls(),
        ...
        resetFileInput(); // Resetta l'input dopo l'importazione
      } else {
