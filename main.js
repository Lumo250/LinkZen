...
// ==============================================
// FUNZIONALITÀ STANDALONE MODE (schermo intero)
// ==============================================
function setupStandaloneFeatures() {
  const isStandalone = window.navigator.standalone || window.matchMedia('(display-mode: standalone)').matches;
  const navBar = document.getElementById("manual-nav-bar");
  const safariMessage = document.getElementById("safari-message"); // Assicurati che esista nel tuo HTML

  if (isStandalone) {
    navBar.classList.remove("hidden");
  } else if (safariMessage) {
    safariMessage.style.display = "block";
    return; // Esce se non in standalone
  }

  let currentUrl = null;
  const urlInput = document.getElementById("manual-nav-url");
  const goBtn = document.getElementById("manual-nav-go");
  const saveBtn = document.getElementById("manual-nav-save");
  ...
  goBtn.addEventListener("click", () => {
    ...
    try {
      ...
      saveBtn.style.display = "inline-block"; // Mostra il pulsante save
      openLinkSafari(currentUrl); // Usa la tua funzione esistente
    } catch (e) {
      console.error("Invalid URL:", e);
      showAlert("Invalid URL", "Please enter a valid web address");
    }
  });
  ...
  saveBtn.addEventListener("click", async () => {
    if (!currentUrl) {
      showAlert("Nothing to save", "Please open a link first.");
      return;
    }
    
    try {
      const title = new URL(currentUrl).hostname.replace(/^www\./, "");
      await processNewLink(currentUrl, title); // Usa la funzione ESISTENTE di LinkZen
      
      // Reset UI
      urlInput.value = "";
      saveBtn.style.display = "none";
      currentUrl = null;
      showAlert("Saved", "Link has been added to your list.");
    } catch (e) {
      console.error("Save error:", e);
      showAlert("Error", "Failed to save the link.");
    }
  });

  // Supporta Invio nell'input
