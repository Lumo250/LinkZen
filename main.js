...
// Modifica l'event listener del pulsante CAT
document.getElementById("categories-btn").addEventListener("click", async () => {
  ...
  // Se la finestra CAT è già visibile, la chiudiamo e usciamo
  if (isVisible) {
    ctBox.classList.add("hidden");
    ctBtn.classList.remove("active");
    return;
  ...
  // Chiudiamo la finestra IA se aperta
  if (!iaBox.classList.contains("hidden")) {
    iaBox.classList.add("hidden");
    iaBtn.classList.remove("active");
  }

  // Apriamo la finestra categorie
  const { userCategories = [] } = await storage.get({ userCategories: [] });
  ...
  defaultTitle.innerHTML = "📁 Default Categories";
  ctBox.appendChild(defaultTitle);

  const defaultContainer = document.createElement("div");
  defaultContainer.style.marginBottom = "20px";
  
  defaultCategories.forEach(category => {
    const chip = document.createElement("span");
    chip.className = "category-chip default-category";
  ...
  customTitle.innerHTML = "📁 Custom Categories";
