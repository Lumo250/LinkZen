...
async function scanQRCode() {
  ...
  try {
    ...
    // GRAFICA ORIGINALE CON AGGIUNTA DEL TIMER
    ...
    try {
      ...
      return new Promise((resolve) => {
        ...
        const updateTimer = () => {
          ...
          const remaining = Math.ceil((QR_SCAN_TIMEOUT - (Date.now() - startTime)) / 1000);
        ...
        // Vibrazione migliorata per iOS
        const vibrateIOS = () => {
            // Primo tentativo: API standard
            if (navigator.vibrate) {
                navigator.vibrate([100, 30, 100]);
                return;
            }
            
            // Secondo tentativo: API webkit per Safari
            if (navigator.webkitVibrate) {
                navigator.webkitVibrate([100, 30, 100]);
                return;
            }
            
            // Terzo tentativo: fallback per iOS < 9
