...
async function showCategories() {
  ...
  const createNewCategoryChip = () => {
    ...
    // Salva quando perde il focus o con Enter
    const saveCategory = async () => {
      const newName = newChip.textContent.trim();
      if (newName && newName !== "New Category") {
        const { userCategories = [] } = await storage.get({ userCategories: [] });
        if (!userCategories.includes(newName)) {
          const updated = [...userCategories, newName];
          await storage.set({ userCategories: updated });
          await showCategories(); // Ricarica la lista
        }
      } else {
        newChip.remove(); // Rimuovi se vuoto o non modificato
      }
    };
  ...
  // Aggiungi gestione click sul pulsante +
  addButton.addEventListener("click", () => {
    const newChip = createNewCategoryChip();
