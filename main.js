...
function preventBodyScroll(prevent) {
  ...
  if (prevent) {
    // Salva la posizione corrente per il ripristino
    body.dataset.savedScrollY = scrollY;
    body.dataset.savedScrollX = scrollX;

    // Blocca lo scroll in tutte le direzioni
    ...
    // Aggiungi gestione specifica per iOS
    ...
    const savedY = parseInt(body.dataset.savedScrollY || '0');
    const savedX = parseInt(body.dataset.savedScrollX || '0');
    window.scrollTo(savedX, savedY);

    // Rimuovi il listener per iOS
    if (/iPad|iPhone|iPod/.test(navigator.userAgent)) {
...
// Sostituisci la parte relativa all'addBtn.addEventListener("click") con questo:
addBtn.addEventListener("click", () => {
  // Blocca lo scroll prima di creare il nuovo elemento
  preventBodyScroll(true);
  document.body.classList.add('modal-open');
  ...
  // Disabilita il drag durante l'edit
  newChip.draggable = false;

  const activateEditing = () => {
    // Focus sull'elemento senza triggerare scroll
    setTimeout(() => {
      newChip.focus();
  ...
  newChip.addEventListener("touchend", (e) => {
    e.preventDefault();
    activateEditing();
  });

  const saveCategory = async () => {
    // Riabilita lo scroll dopo il salvataggio
    preventBodyScroll(false);
    document.body.classList.remove('modal-open');
