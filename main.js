...
document.getElementById("ia-knowledge-btn").addEventListener("click", async () => {
  ...
  // Chiudi la finestra categorie se aperta
  ctBox.classList.add("hidden");
  ctBtn.classList.remove("active");

  // Il resto del codice esistente per popolare la IA box...
...
document.getElementById("categories-btn").addEventListener("click", async () => {
  ...
  // Il resto del codice esistente per popolare la categories box...
  const { userCategories = [] } = await storage.get({ userCategories: [] });
