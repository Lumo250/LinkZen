...
let lastCategoryInputClickTime = 0;
let isCategoryInputFocused = false;
...
newCategoryInput.addEventListener('click', (e) => {
  const now = Date.now();
  const popup = document.getElementById('categories-popup');
  const wasRecentlyClicked = (now - lastCategoryInputClickTime) < 300; // 300ms threshold
  
  // Se è un click rapido consecutivo e non c'è testo
  if (wasRecentlyClicked && isCategoryInputFocused && !newCategoryInput.value.trim()) {
    // Chiudi tutto
  ...
  lastCategoryInputClickTime = now;
});

// Aggiungi questo per gestire quando la tastiera viene chiusa manualmente
newCategoryInput.addEventListener('blur', () => {
  isCategoryInputFocused = false;
});

// Mantieni gli altri event listener
