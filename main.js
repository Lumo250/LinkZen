...
function createEditableCategoryChip(container) {
  ...
  // Funzione per salvare la categoria
  const save = async () => {
    ...
    if (!userCategories.includes(newCat)) {
      ...
      await showCategories(); // ricarica l'elenco
      await loadUrls();       // aggiorna i link filtrati
    } else {
      // già esiste: chiudi senza aggiungere doppioni
      container.removeChild(chip);
    }
  };
...
function highlightSimilarCategories(inputValue, container) {
  ...
  chips.forEach(chip => {
    ...
    if (!inputValue || !lowerName.includes(lowerInput)) {
      ...
      // Evidenzia la parte corrispondente in giallo
      ...
      chip.style.backgroundColor = "#fffde7"; // tenue giallo chiaro
    }
  });
}
