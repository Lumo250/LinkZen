...
// Sostituisci l'attuale event listener 'focus' con questo:
newCategoryInput.addEventListener('focus', (e) => {
  const popup = document.getElementById('categories-popup');
  const isPopupVisible = !popup.classList.contains('hidden');
  
  // Se il popup è già visibile e non c'è testo nell'input, chiudi tutto
  if (isPopupVisible && !newCategoryInput.value.trim()) {
    // Forza la chiusura della tastiera mobile
    newCategoryInput.blur();
    hideCategoriesPopup();
  } else {
    // Altrimenti mostra il popup normalmente
    popup.classList.remove('hidden');
