...
function showBookmarkletInstructions() {
  // 1. Ottieni l'URL base della PWA in modo dinamico
  const pwaUrl = window.location.origin + window.location.pathname.split('/').slice(0, 2).join('/');

  // 2. Genera il codice del bookmarklet (versione compressa e codificata)
  const bookmarkletCode = `javascript:(function(){ localStorage.setItem('pendingBookmarkletUrl',location.href); localStorage.setItem('pendingBookmarkletTitle',document.title); window.location.href='${pwaUrl}'; })();`;

  // 3. Mostra le istruzioni con pulsante di copia
