...
async function processBookmarkletRequest() {
 ...
 // Prima controlla localStorage per link pendenti
  const pendingUrl = localStorage.getItem('pendingUrl');
  const pendingTitle = localStorage.getItem('pendingTitle');
  
  if (pendingUrl) {
    try {
      const decodedUrl = decodeURIComponent(pendingUrl);
      const decodedTitle = decodeURIComponent(pendingTitle || '');
  ...
  // Poi controlla i parametri URL (mantieni la logica esistente)
...
// Prova a navigare nella stessa finestra
  try {
...
document.addEventListener("DOMContentLoaded", async () => {
 ...
 // Ripristina lo stato se siamo tornati da una navigazione
  ...
  if (pwaState) {
    ...
    // Aggiorna lo stato del link come "cliccato"
    if (lastOpenedUrl) {
...
// Processa eventuali link in attesa dal bookmarklet
  const pendingUrl = localStorage.getItem('pendingUrl');
  const pendingTitle = localStorage.getItem('pendingTitle');
