...
async function processBookmarkletRequest() {
  try {
    // Leggi i dati salvati dal bookmarklet
    const pendingData = localStorage.getItem('pendingBookmarkletData');
    if (!pendingData) return;
    
    const { url, title, timestamp } = JSON.parse(pendingData);
    
    // Pulisci subito il localStorage
    localStorage.removeItem('pendingBookmarkletData');
    
    // Verifica che i dati non siano troppo vecchi (max 1 minuto)
    if (Date.now() - timestamp > 60000) {
      console.log('Bookmarklet data expired');
      return;
    }
    ...
    // Scrolla al nuovo elemento
    const lastLink = document.querySelector(`a[href="${decodedUrl}"]`);
    if (lastLink) {
...
function openLinkSafari(url) {
 // Salva lo stato prima di navigare
  sessionStorage.setItem('pwaState', JSON.stringify({
    scrollPosition: window.scrollY,
    lastOpenedUrl: url
  }));
  
  // Usa un approccio che funziona meglio con iOS
  try {
    // Prova prima con window.open (funziona meglio in alcune PWAs)
...
document.addEventListener("DOMContentLoaded", async () => {
  // Ripristina lo stato se siamo tornati da una navigazione
  const pwaState = sessionStorage.getItem('pwaState');
  if (pwaState) {
    const { scrollPosition, lastOpenedUrl } = JSON.parse(pwaState);
    sessionStorage.removeItem('pwaState');
    window.scrollTo(0, scrollPosition);
    
    if (lastOpenedUrl) {
      const { clickedUrls = [] } = await storage.get({ clickedUrls: [] });
      if (!clickedUrls.includes(lastOpenedUrl)) {
        await storage.set({ clickedUrls: [...clickedUrls, lastOpenedUrl] });
        await loadUrls();
      }
    }
  }

  // Processa eventuali dati dal bookmarklet
  await processBookmarkletRequest();

  
  if (pendingUrl) {
    await processNewLink(decodeURIComponent(pendingUrl), decodeURIComponent(pendingTitle));
    ...
    localStorage.removeItem('pendingBookmarkletUrl');
    localStorage.removeItem('pendingBookmarkletTitle');
...
function showBookmarkletInstructions() {
  // 1. Codice bookmarklet aggiornato
  const bookmarkletCode = `javascript:(function(){
    // Salva i dati in modo più persistente
    const data = {
    ...
    localStorage.setItem('pendingBookmarkletData', JSON.stringify(data));
    
    // Forza il caricamento della PWA in un contesto fresco
    const pwaUrl = 'https://lumo250.github.io/LinkZen/';
    if (window.location.href.startsWith(pwaUrl)) {
      // Se già nella PWA, ricarica forzatamente
      window.location.href = pwaUrl + '?forceReload=' + Date.now();
    } else {
      // Altrimenti naviga alla PWA
      window.location.href = pwaUrl;
    }
  })();`;

  // 2. Mostra la finestra con pulsante di copia migliorato
  showAlert("How to Install Bookmarklet", `
    <div style="font-size:14px; line-height:1.5">
      1. <strong>Copy this entire code:</strong>
      <div style="position:relative; margin:10px 0">
        <textarea id="bookmarklet-code" 
          style="
            ...
            resize:none;
          "
          readonly>${bookmarkletCode}</textarea>
        <button id="copy-bookmarklet-btn" 
          style="
            position:absolute;
            top:8px;
            right:8px;
      ...
      2. <strong>Installation:</strong>
      <div style="margin-top:8px; padding-left:10px; border-left:2px solid #4299E1">
        <div style="margin-bottom:8px">
          <strong>Safari iOS:</strong><br>
          • Tap <span style="color:#4299E1">Share</span> → <span style="color:#4299E1">Add Bookmark</span><br>
          • Delete the URL field<br>
          • Paste the copied code<br>
          • Save as "Save to LinkZen"
        </div>
        <div>
          <strong>Desktop Browsers:</strong><br>
          • Drag this link to bookmarks bar: 
          <a href="${bookmarkletCode}" style="color:#4299E1; word-break:break-all">Save to LinkZen</a>
        </div>
      </div>
    </div>
  `);

  // 3. Aggiungi funzionalità di copia avanzata
...
try {
  ...
  // Apre nella stessa finestra dopo aver aggiornato lo stato
  openLinkSafari(url);
});
