...
await storage.set({ sortOrder: newSort });
  ...
  // Gestisci la visualizzazione
  const iconContainer = document.getElementById('icon-view-container');
  const listContainer = document.getElementById('url-container');
...
async function showIconView() {
  const iconContainer = document.getElementById('icon-view-container');
  const listContainer = document.getElementById('url-container');
  
  // Mostra la vista icone e nascondi la lista
  iconContainer.classList.remove('hidden');
  iconContainer.classList.add('visible');
  listContainer.classList.add('hidden');
  
  const { visitedUrls = [], clickedUrls = [] } = await storage.get({ 
    visitedUrls: [], 
    clickedUrls: [] 
  });
  
  // Pulisci il container solo se non è già stato popolato
  if (iconContainer.children.length === 0) {
    ...
    visitedUrls.forEach(item => {
      ...
      img.src = `https://www.google.com/s2/favicons?domain=${encodeURIComponent(item.url)}&sz=64`;
      img.alt = item.title || '';
      img.loading = 'lazy';
      ...
      title.textContent = item.title || new URL(item.url).hostname.replace('www.', '');
      
      iconItem.appendChild(imgContainer);
      ...
      iconItem.addEventListener('click', (e) => {
        ...
        if (!clickedUrls.includes(item.url)) {
          const newClickedUrls = [...clickedUrls, item.url];
          storage.set({ clickedUrls: newClickedUrls });
          
          // Aggiungi effetto visivo al click
...
// Aggiungi questo alla fine del tuo main.js
document.querySelectorAll('.close-modal-btn').forEach(btn => {
  btn.addEventListener('click', function() {
    // Chiudi la modale genitore
    const modal = this.closest('#ia-knowledge-box, #categories-box');
    modal.classList.add('hidden');
    
    // Rimuovi lo stato "active" dal pulsante corrispondente
    if(modal.id === 'ia-knowledge-box') {
      document.getElementById('ia-knowledge-btn').classList.remove('active');
    } else {
      document.getElementById('categories-btn').classList.remove('active');
    }
