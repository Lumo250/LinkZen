...
async function renderCategoriesList(searchTerm = '') {
 ...
 const filtered = searchTerm 
  ? userCategories.filter(cat => 
    ...
    ) // Parentesi mancante aggiunta qui
  : userCategories;
  
  if (filtered.length === 0) {async function renderCategoriesList(searchTerm = '') {
  const { userCategories = [] } = await storage.get({ userCategories: [] });
  const list = document.getElementById('categories-list');
  list.innerHTML = '';
  ...
  filtered.forEach(cat => {
    ...
    deleteBtn.addEventListener('click', async (e) => {
      ...
      renderCategoriesList(document.getElementById('category-search').value); // Modifica questa linea
});
...
async function deleteCategory(category) {
  ...
  // Aggiorna sia la lista popup che la vista principale
  await renderCategoriesList(document.getElementById('category-search').value);
  await renderSelectedCategories(); // Aggiungi questa linea
  await updateCategorySuggestions();
  await loadUrls();
