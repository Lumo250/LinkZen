...
// --- BLOCCO 1: showCategories() aggiornata con pulsante [+] ---
async function showCategories() {
  const ctBox = document.getElementById("categories-box");
  if (!ctBox) return; // sicurezza
  ctBox.innerHTML = '';
  ...
  addBtn.addEventListener("click", () => {
    createEditableCategoryChip(customContainer);
  });
...
// --- BLOCCO 2: funzione di inserimento inline ---
function createEditableCategoryChip(container) {
  const chip = document.createElement("span");
  chip.className = "category-chip custom-category";
  ...
  input.setAttribute("aria-label", "Enter new category");

  // Evidenziazione mentre si digita
  input.addEventListener("input", () => {
    highlightSimilarCategories(input.value, container);
  });
  ...
  const save = async () => {
    ...
    if (!newCat) {
      ...
      highlightSimilarCategories("", container);
      return;
    }

    const { userCategories = [] } = await storage.get({ userCategories: [] });
    if (!userCategories.includes(newCat)) {
      await storage.set({ userCategories: [...userCategories, newCat] });
      await showCategories();
      await loadUrls();
    } else {
      container.removeChild(chip);
...
// --- BLOCCO 3: evidenziazione dinamica delle categorie ---
function highlightSimilarCategories(inputValue, container) {
  ...
  chips.forEach(chip => {
    ...
    if (!inputValue || !lowerName.includes(lowerInput)) {
      chip.innerHTML = name;
      chip.style.backgroundColor = "";
    } else {
      const regex = new RegExp(`(${lowerInput})`, 'i');
      const highlighted = name.replace(regex, '<mark style="background: yellow; color: inherit;">$1</mark>');
      chip.innerHTML = highlighted;
      chip.style.backgroundColor = "#fffde7";
    }
  });
}
