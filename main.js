...
async function showCategories() {
  ...
  // Aggiungi titoli con pulsante [+] per le categorie custom
  const customTitle = document.createElement("div");
  customTitle.style.display = "flex";
  customTitle.style.alignItems = "center";
  customTitle.style.justifyContent = "space-between";
  customTitle.style.marginBottom = "8px";
  
  const titleText = document.createElement("h3");
  titleText.className = "category-section-title";
  titleText.textContent = "📁 Custom Categories";
  
  const addButton = document.createElement("button");
  addButton.innerHTML = "+";
  addButton.style.background = "transparent";
  addButton.style.border = "none";
  addButton.style.color = document.body.classList.contains("dark") ? "#aaa" : "#555";
  ...
  addButton.addEventListener("mouseenter", () => {
    addButton.style.backgroundColor = document.body.classList.contains("dark") ? "#444" : "#eee";
  });
  ...
  customTitle.appendChild(titleText);
  customTitle.appendChild(addButton);

  // Titolo per le categorie default (senza pulsante)
  ...
  const createCategoryChip = (name, isCustom) => {
    ...
    chip.draggable = true;
    chip.dataset.categoryName = name;
    ...
    // Aggiungi doppio click per modificare (solo categorie custom)
    if (isCustom) {
      chip.addEventListener('dblclick', (e) => {
        e.stopPropagation();
        showCategoryEditor(name, true);
      });
    }
...
// Nuova funzione per l'editor di categorie
function showCategoryEditor(currentName = "", isCustom = true) {
  ...
  editor.innerHTML = `
    ...
    ">
      <h3 style="margin-top:0">${currentName ? "Edit Category" : "New Category"}</h3>
      <input type="text" id="category-editor-input" 
             value="${currentName}" 
             ...
             ">
      <div id="matching-categories" style="
        max-height: 150px;
        overflow-y: auto;
        margin-bottom: 15px;
        border: 1px solid ${isDark ? "#4a5568" : "#e2e8f0"};
      ...
      <div style="display:flex;justify-content:flex-end;gap:10px;">
        <button id="cancel-category-edit" style="
          padding: 8px 16px;
        ...
        <button id="save-category-edit" style="
          padding: 8px 16px;
  ...
  const input = editor.querySelector("#category-editor-input");
  input.focus();
  if (currentName) input.select();
  
  // Funzione per evidenziare le categorie corrispondenti
  const highlightMatchingCategories = async () => {
    const value = input.value.toLowerCase();
    const matchingDiv = editor.querySelector("#matching-categories");
    ...
    const matches = userCategories.filter(cat => 
      cat.toLowerCase().includes(value) && cat !== currentName
    );
    
    if (matches.length === 0) {
      matchingDiv.textContent = "No matching categories";
      matchingDiv.style.color = isDark ? "#a0aec0" : "#718096";
      matchingDiv.style.fontStyle = "italic";
      matchingDiv.style.padding = "8px";
      return;
    }
    
    matches.forEach(cat => {
      const matchDiv = document.createElement("div");
      matchDiv.textContent = cat;
      matchDiv.style.padding = "6px";
      matchDiv.style.margin = "2px 0";
      matchDiv.style.backgroundColor = isDark ? "rgba(255,255,0,0.1)" : "rgba(255,255,0,0.3)";
      matchDiv.style.borderRadius = "4px";
      matchDiv.style.borderLeft = `3px solid ${isDark ? "yellow" : "gold"}`;
      matchingDiv.appendChild(matchDiv);
    });
  };
  
  input.addEventListener("input", highlightMatchingCategories);
  
  // Salva la categoria
  editor.querySelector("#save-category-edit").addEventListener("click", async () => {
    ...
    if (currentName) {
      // Modifica esistente
      const updatedUserCats = userCategories.map(c => 
        c === currentName ? newName : c
      );
      
      const updatedUrls = visitedUrls.map(link => {
        if (link.category === currentName) {
      ...
      // Nuova categoria
      if (!userCategories.includes(newName)) {
        await storage.set({
          userCategories: [...userCategories, newName]
        });
      }
    }
  ...
  // Annulla
  editor.querySelector("#cancel-category-edit").addEventListener("click", () => {
    document.body.removeChild(editor);
  });
  ...
  input.addEventListener("keydown", (e) => {
    if (e.key === "Escape") {
      ...
      editor.querySelector("#save-category-edit").click();
    }
  });
}
