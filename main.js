...
function applyFontSize(scale) {
  ...
  // Applica a entrambe le info box
  document.getElementById("ia-knowledge-box").style.fontSize = `${scale}em`;
  document.getElementById("categories-box").style.fontSize = `${scale}em`;
}
...
async function showCategories() {
  ...
  defaultTitle.textContent = "📁 Default Categories (trascina in Custom per aggiungerle)";
  
  const customTitle = document.createElement("h3");
  customTitle.className = "category-section-title";
  customTitle.textContent = "📁 Custom Categories (trascina in Default per rimuoverle)";
  ...
  // Mostra solo le default che non sono già custom
  ...
  const setupDropZone = (element, willBecomeCustom) => {
    ...
    element.addEventListener('drop', async (e) => {
      ...
      // Se già nello stato target, ignora
      ...
      // Aggiorna le URL che usavano questa categoria
      const updatedUrls = visitedUrls.map(link => {
        if (link.category === name) {
          return {
            ...link,
            category: willBecomeCustom ? name : (link.originalCategory || "Other")
          };
        }
        return link;
      });
      
      await storage.set({ visitedUrls: updatedUrls });
...
try {
   ...
   // Modifica questa parte per gestire sia default che custom categories
  const { defaultCategories = [] } = await getDefaultCategories();
  const { userCategories = [] } = await storage.get({ userCategories: [] });
  
  // Combina le categorie per il dropdown
