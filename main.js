...
async function showCategories() {
  ...
  // Aggiungi titoli con pulsante [+] solo per le categorie custom
  ...
  // Titolo per le categorie default
  ...
  // Setup drop zones (mantieni lo stesso codice esistente)
  ...
  // Aggiungi gestore eventi per il pulsante +
  addButton.addEventListener('click', () => {
    showCategoryEditor();
  });
}

// Funzione per mostrare l'editor di categoria
function showCategoryEditor(existingName = "") {
  const editor = document.createElement("div");
  editor.className = "category-editor-overlay";
  
  editor.innerHTML = `
    <div class="category-editor">
      <h3>${existingName ? "Edit Category" : "New Category"}</h3>
      <input type="text" class="category-editor-input" 
             value="${existingName}" 
  ...
  editor.querySelector(".save-btn").addEventListener('click', async () => {
    ...
    if (existingName) {
      ...
      const updatedUserCats = userCategories.map(c => 
        c === existingName ? newName : c
      );
      
      const updatedUrls = visitedUrls.map(link => {
        if (link.category === existingName) {
  ...
  input.addEventListener('keydown', (e) => {
    if (e.key === "Escape") {
      ...
      editor.querySelector(".save-btn").click();
    }
  });
}
