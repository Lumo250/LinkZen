...
preventBodyScroll(true);
  ...
  // Se la finestra IA è già visibile, la chiudiamo
  if (isVisible) {
    iaBox.classList.add("hidden");
    iaBtn.classList.remove("active");
    preventBodyScroll(false); // <-- Aggiunto qui
    document.body.classList.remove('modal-open'); // <-- Aggiunto qui
    return;
  }

  // Altrimenti, apriamo la finestra IA
  ...
  // Nota: queste chiamate di sblocco non dovrebbero essere qui per l'apertura
   preventBodyScroll(false); // <-- Rimuovi queste righe
   document.body.classList.remove('modal-open'); // <-- Rimuovi queste righe
});
