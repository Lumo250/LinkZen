...
sortToggle.addEventListener("click", async () => {
  ...
  // Rimuovi tutte le classi di stato e aggiungi quella corretta
  sortToggle.classList.remove("active", "custom");
  if (newClass) sortToggle.classList.add(newClass);
  
  // Applica l'ordinamento SOLO per "default" e "category"
  if (newSort !== "custom") {
    await loadUrls();
  }
});
...
sortToggle.textContent = 
  sortOrder === "default" ? "By Input" : 
  sortOrder === "category" ? "By Category" : "By Custom";
sortToggle.className = "single-pill-option " + 
  (sortOrder === "default" ? "" : 
   sortOrder === "category" ? "active" : "custom");
