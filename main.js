...
function toggleKnowledgeBoxes(activeButton) {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (isVisible) {
    toggleKnowledgeBoxes(null);
  } else {
    toggleKnowledgeBoxes('ia');
    
    // Il resto del codice per popolare la box IA rimane uguale
...
document.getElementById("categories-btn").addEventListener("click", async () => {
  ...
  if (isVisible) {
    ...
    toggleKnowledgeBoxes('ct');
    
    // Il resto del codice per popolare la box categorie rimane uguale
    const { userCategories = [] } = await storage.get({ userCategories: [] });
