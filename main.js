...
localStorage.setItem("defaultCategories", JSON.stringify(defaultCategories));
  ...
  // Zoom
// Zoom
if (!window.zoomListenersInitialized) {
  window.zoomListenersInitialized = true;
...
async function showCategories() {
  ...
  const setupDropZone = (element, willBecomeCustom) => {
    ...
    element.addEventListener('drop', async (e) => {
      ...
      const draggedChip = document.querySelector('.dragging');
      if (draggedChip) {
        draggedChip.classList.remove('dragging');
        element.appendChild(draggedChip);

        // Cambia classe in base al tipo
        if (willBecomeCustom) {
          draggedChip.classList.remove('default-category');
          draggedChip.classList.add('custom-category');
        } else {
          draggedChip.classList.remove('custom-category');
          draggedChip.classList.add('default-category');
        }
      }
    });
  };
