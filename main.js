...
async function showCategories() {
  ...
  // Crea i contenitori per le categorie
  ...
  // Crea una dropzone
  const createDropzone = (title, isCustomTarget) => {
    const container = document.createElement("div");
    container.className = "category-dropzone";
    
    const titleElement = document.createElement("div");
    titleElement.className = "category-section-title";
    ...
    chipsContainer.style.padding = "8px";
    chipsContainer.style.borderRadius = "4px";

    // Gestione eventi dropzone
    chipsContainer.addEventListener('dragover', (e) => {
      e.preventDefault();
      chipsContainer.classList.add('highlight');
    });

    chipsContainer.addEventListener('dragleave', () => {
      chipsContainer.classList.remove('highlight');
    });
    ...
    chipsContainer.addEventListener('drop', async (e) => {
      ...
      // Se la categoria è già nel target, esci
      if ((isCustomTarget && isCustom) || (!isCustomTarget && !isCustom)) return;

      // Aggiorna le categorie utente
      let updatedUserCats;
      if (isCustomTarget) {
        // Aggiungi alla lista custom
      ...
      const updatedUrls = visitedUrls.map(link => {
        if (link.category === name) {
          return {
            ...link,
            originalCategory: isCustomTarget ? link.originalCategory : name
          };
        }
        return link;
      });
    ...
    return { container, chipsContainer };
  };

  // Crea le due sezioni
  const defaultSection = createDropzone("📁 Default Categories", false);
  const customSection = createDropzone("📁 Custom Categories", true);

  // Popola le categorie default
  DEFAULT_CATEGORIES.forEach(cat => {
    if (!userCategories.includes(cat)) {
      defaultSection.chipsContainer.appendChild(createCategoryChip(cat, false));
    }
  });

  // Popola le categorie custom
  if (userCategories.length === 0) {
    const emptyMsg = document.createElement("div");
    emptyMsg.textContent = "Nessuna categoria personalizzata. Aggiungine una usando l'input sopra.";
    emptyMsg.style.color = document.body.classList.contains("dark") ? "#aaa" : "#666";
    customSection.chipsContainer.appendChild(emptyMsg);
  } else {
    userCategories.forEach(cat => {
      customSection.chipsContainer.appendChild(createCategoryChip(cat, true));
    });
  }

  // Aggiungi tutto alla schermata
  ctBox.appendChild(defaultSection.container);
  ctBox.appendChild(customSection.container);
}
...
document.getElementById("categories-btn").addEventListener("click", async () => {
  ...
  // Chiudi la finestra IA se aperta
  if (!iaBox.classList.contains("hidden")) {
    iaBox.classList.add("hidden");
    iaBtn.classList.remove("active");
  }

  // Mostra la finestra categorie
  await showCategories();
...
async function getDefaultCategories() {
  try {
    ...
    if (response.ok) {
      return { defaultCategories: await response.json() };
    }
  } catch (error) {
