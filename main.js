...
// Barra manuale solo in modalità standalone
...
if (isStandalone) {
  ...
  const input = document.getElementById("manual-nav-url");
  ...
  goBtn.addEventListener("click", () => {
    ...
    if (!isValidUrl(url)) {
      showAlert("Invalid URL", "Please enter a valid http or https address.");
      return;
    }

    lastManualUrl = url;
    window.location.href = url; // ✅ Sostituisce openLinkSafari qui
  });
  ...
  saveBtn.addEventListener("click", async () => {
    ...
    try {
      await processNewLink(lastManualUrl, lastManualUrl);
      showAlert("Saved", "Link has been added to your list.");
    } catch (error) {
      console.error("Error saving manual link:", error);
