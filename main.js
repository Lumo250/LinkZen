...
function applyFontSize(scale) {
  ...
  // Imposta lo stile solo se esiste il box
  const iaBox = document.getElementById("ia-knowledge-box");
  const ctBox = document.getElementById("categories-box");

  if (iaBox) iaBox.style.fontSize = `${scale}em`;
  if (ctBox && !ctBox.classList.contains("reapplying-font")) {
    ctBox.classList.add("reapplying-font");
    ctBox.style.fontSize = `${scale}em`;
    // Rimuovi subito dopo per non interferire con altre logiche
    setTimeout(() => ctBox.classList.remove("reapplying-font"), 50);
  }
}
