...
// Sostituisci la funzione showCategories con questa versione migliorata
async function showCategories() {
  ...
  // Aggiungi editor per nuova categoria
  const editorContainer = document.createElement("div");
  editorContainer.className = "category-editor";
  ...
  customTitle.innerHTML = "📁 Custom Categories <button class='add-category-btn'>+</button>";

  // Funzione per creare una chip editabile
  const createCategoryChip = (name, isCustom, isEditing = false) => {
    const chip = document.createElement("div");
    chip.className = `category-item ${isCustom ? 'custom-category' : 'default-category'} ${isEditing ? 'editing' : ''}`;
    chip.draggable = !isEditing;
    
    if (isEditing) {
      chip.innerHTML = `
        <input type="text" value="${name}" class="edit-category-input" style="flex-grow:1;padding:4px;">
        <button class="save-edit-btn" style="margin-left:6px;padding:4px 8px;">Save</button>
        <button class="cancel-edit-btn" style="margin-left:4px;padding:4px 8px;">Cancel</button>
      `;
    } else {
      chip.innerHTML = `
        <span class="category-name">${name}</span>
        ${isCustom ? '<span class="delete-category">&times;</span>' : ''}
      `;
    }
  ...
  const setupDropZone = (element, willBecomeCustom) => {
    ...
    element.addEventListener('drop', async (e) => {
      ...
      await storage.set({ userCategories: updatedUserCats });
      localStorage.setItem("defaultCategories", JSON.stringify(updatedDefaultCats));
      await showCategories();
    });
  };

  // Setup event listeners per editing
  const setupEditing = () => {
    ...
    document.getElementById('add-category-btn').addEventListener('click', async () => {
      ...
      if (newName && !userCategories.includes(newName)) {
        const updated = [...userCategories, newName];
        await storage.set({ userCategories: updated });
        input.value = '';
        await showCategories();
      }
    ...
    // Input per nuova categoria - evidenzia corrispondenze durante la digitazione
    document.getElementById('new-category-name').addEventListener('input', (e) => {
      ...
      if (searchText.length > 0) {
        document.querySelectorAll('.category-item .category-name').forEach(item => {
          if (item.textContent.toLowerCase().includes(searchText)) {
            item.closest('.category-item').classList.add('highlight-match');
          }
        });
      }
    });

    // Enter per aggiungere nuova categoria
    document.getElementById('new-category-name').addEventListener('keydown', async (e) => {
      if (e.key === 'Enter') {
        const input = document.getElementById('new-category-name');
        const newName = input.value.trim();
    ...
    document.querySelectorAll('.category-name').forEach(nameEl => {
      nameEl.addEventListener('click', (e) => {
        ...
        // Sostituisci con input di editing
        const newChip = createCategoryChip(currentName, true, true);
        chip.parentNode.replaceChild(newChip, chip);
        
        // Focus sull'input
        const input = newChip.querySelector('.edit-category-input');
        input.focus();
        input.select();
        
        // Salva modifica
        newChip.querySelector('.save-edit-btn').addEventListener('click', async () => {
          const newName = input.value.trim();
          if (newName && newName !== currentName) {
            ...
            const updatedUserCats = userCategories.map(c => c === currentName ? newName : c);
            const updatedUrls = visitedUrls.map(link => {
        ...
        // Annulla modifica
        newChip.querySelector('.cancel-edit-btn').addEventListener('click', async () => {
          await showCategories();
        });
    ...
    document.querySelectorAll('.delete-category').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        const updatedUserCats = userCategories.filter(c => c !== categoryName);
        const updatedUrls = visitedUrls.map(link => {
  ...
  ctBox.appendChild(editorContainer);
  ctBox.appendChild(customContainer);

  // Setup event listeners dopo che il DOM è stato renderizzato
  setTimeout(setupEditing, 0);
}
