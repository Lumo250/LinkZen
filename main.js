...
// Controlla se c'è un link in attesa dal bookmarklet
  const pendingUrl = localStorage.getItem('pendingBookmarkletUrl');
  const pendingTitle = localStorage.getItem('pendingBookmarkletTitle');
  
  if (pendingUrl && pendingTitle) {
    // Processa il salvataggio
    await processNewLink(pendingUrl, pendingTitle);
...
function showBookmarkletInstructions() {
  // 1. Ottieni l'URL base della PWA
  const pwaUrl = window.location.origin + window.location.pathname.split('/').slice(0, 2).join('/');

  // 2. Genera il codice del bookmarklet (VERSIONE CORRETTA)
  const rawBookmarklet = `(function(){
    localStorage.setItem('pendingBookmarkletUrl', location.href);
    localStorage.setItem('pendingBookmarkletTitle', document.title);
    window.location.href = '${pwaUrl}';
  })()`;

  // 3. Codifica per URL (fondamentale!)
  const encodedBookmarklet = encodeURIComponent(rawBookmarklet)
    .replace(/'/g, '%27')
    .replace(/"/g, '%22');

  const bookmarkletCode = `javascript:${encodedBookmarklet}`;

  // 4. Mostra il testo copiabile
  showAlert("How to Install Bookmarklet", `
    1. <strong>Copy this entire code:</strong><br><br>
    <textarea id="bookmarklet-code" style="
      width: 100%;
      padding: 10px;
      border-radius: 6px;
      background: #4A5568;
      ...
      min-height: 60px;
      resize: none;
    " readonly>${bookmarkletCode}</textarea><br>
    ...
    2. <strong>In Safari:</strong><br>
    - Tap su <img src="https://cdn-icons-png.flaticon.com/512/57/57055.png" width="14" height="14" style="vertical-align:middle"> > "Add to Bookmarks"<br>
    - Incolla nel campo "URL"
  `);
}
