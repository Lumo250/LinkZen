...
resetBtn.title = "Restore default categories";

resetBtn.addEventListener("click", async () => {
  const confirmed = confirm("Are you sure you want to reset all default categories?\nThis will delete any category you've added.");
  if (!confirmed) return;

  await storage.set({ userCategories: [] });
  await showCategories();
  await loadUrls?.(); // se esiste, aggiorna anche l'elenco link
});
