...
// ==============================================
// QR SCANNER FUNZIONANTE SU IOS (TESTATO SU IPHONE SE 2020)
// ==============================================
async function scanQRCode() {
  // 1. Carica la libreria jsQR
  if (!window.jsQR) {
    await loadScript('https://cdn.jsdelivr.net/npm/jsqr@1.4.0/dist/jsQR.min.js');
    if (!window.jsQR) {
      showAlert("Error", "QR scanner failed to load");
      return null;
    }
  }

  // 2. Crea l'interfaccia scanner
  const scannerDiv = createScannerUI();
  ...
  try {
    ...
    // 4. Configura il canvas per l'analisi
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d', { willReadFrequently: true });
    let scanActive = true;

    // 5. Funzione di scansione ottimizzata per iOS
    const scanFrame = () => {
      if (!scanActive) return;

      try {
        if (video.readyState >= video.HAVE_ENOUGH_DATA) {
          // Imposta dimensioni canvas (quadrato centrale)
          const minDimension = Math.min(video.videoWidth, video.videoHeight);
          canvas.width = minDimension;
          canvas.height = minDimension;
          
          // Ritaglia l'area centrale
          const offsetX = (video.videoWidth - minDimension) / 2;
          const offsetY = (video.videoHeight - minDimension) / 2;
          
          ctx.drawImage(video, offsetX, offsetY, minDimension, minDimension, 0, 0, minDimension, minDimension);
          
          // Analizza l'immagine
          const imageData = ctx.getImageData(0, 0, minDimension, minDimension);
          ...
          if (code) {
            handleQRDetected(code.data, stream, scannerDiv);
            return;
          }
        }
    ...
    cancelBtn.addEventListener('click', () => {
      scanActive = false;
      cleanupScanner(stream, scannerDiv);
    });

    // 7. Avvia la scansione
    const scanPromise = new Promise((resolve) => {
      video.onplaying = () => {
        const detectionInterval = setInterval(() => {
          scanFrame();
        }, 300); // Scansione ogni 300ms
      };
    });
    ...
    cleanupScanner(null, scannerDiv);
...
// ==============================================
// FUNZIONI DI SUPPORTO PER LO SCANNER
// ==============================================
...
function createScannerUI() {
  ...
  div.innerHTML = `
    ...
    <div style="position:absolute;top:20px;left:0;right:0;text-align:center;color:white;padding:10px;background:rgba(0,0,0,0.5)">
      ...
      <p style="margin:5px 0 0 0;font-size:0.9rem">Align QR code within frame</p>
    </div>
...
function handleQRDetected(qrData, stream, scannerDiv) {
  // Feedback vibrazione
...
function cleanupScanner(stream, scannerDiv) {
  if (stream) {
    stream.getTracks().forEach(track => track.stop());
  }
  if (scannerDiv && document.body.contains(scannerDiv)) {
    document.body.removeChild(scannerDiv);
  }
}
