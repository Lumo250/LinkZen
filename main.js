...
async function showCategories() {
  ...
  // Titolo + Pulsante per le custom
  const customTitleWrapper = document.createElement("div");
  customTitleWrapper.style.display = "flex";
  customTitleWrapper.style.alignItems = "center";
  customTitleWrapper.style.gap = "8px";
  ...
  Object.assign(addBtn.style, {
    ...
    borderRadius: "50%",
    border: "none",
    background: document.body.classList.contains("dark") ? "#388E3C" : "#4CAF50",
    color: "white",
    fontWeight: "bold",
    cursor: "pointer",
    display: "flex",
...
const createCategoryChip = (name, isCustom) => {
  ...
  // Tasto di cancellazione solo per categorie personalizzate
  if (isCustom) {
    ...
    closeBtn.addEventListener("click", async (e) => {
      e.stopPropagation(); // evita conflitti con il drag
      const { userCategories = [] } = await storage.get({ userCategories: [] });
      const updated = userCategories.filter(c => c !== name);
      await storage.set({ userCategories: updated });
      await showCategories();
      await loadUrls?.(); // se esiste, aggiorna i link
    });
