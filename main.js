...
async function showCategories() {
  ...
  const setupDropZone = (element, willBecomeCustom) => {
    ...
    element.addEventListener('drop', async (e) => {
      ...
      if (willBecomeCustom) {
        // Da default → custom
        updatedUserCats = userCategories.includes(name)
          ? userCategories
          : [...userCategories, name];
        ...
        updatedDefaultCats = defaultCategories.includes(name)
          ? defaultCategories
      ...
      await showCategories(); // ricarica la finestra
    });
  };

  // Applica le drop zone ai contenitori
