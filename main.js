...
importFileInput.addEventListener("change", async (event) => {
  ...
  // 1. Salva lo stato VISIBILE corrente delle finestre
  const iaWasVisible = document.getElementById("ia-knowledge-box").classList.contains("hidden") === false;
  const ctWasVisible = document.getElementById("categories-box").classList.contains("hidden") === false;

  // 2. Chiudi TUTTE le finestre durante l'import
  document.getElementById("ia-knowledge-box").classList.add("hidden");
  document.getElementById("ia-knowledge-btn").classList.remove("active");
  ...
  reader.onload = async (e) => {
    try {
      ...
      if (data.visitedUrls && Array.isArray(data.visitedUrls)) {
        ...
        // 3. Ricarica i dati SENZA modificare la visibilità
        await loadUrls();
        await showCategories();
        await renderIAKeywords();
        await updateCategorySuggestions();
        ...
        // 4. Ripristina SOLO la finestra che era visibile prima
        if (iaWasVisible) {
          document.getElementById("ia-knowledge-box").classList.remove("hidden");
          document.getElementById("ia-knowledge-btn").classList.add("active");
        }
        if (ctWasVisible) {
          document.getElementById("categories-box").classList.remove("hidden");
...
async function renderIAKeywords() {
 ...
 iaBox.classList.remove("hidden"); // Assicurati che la box sia visibile
